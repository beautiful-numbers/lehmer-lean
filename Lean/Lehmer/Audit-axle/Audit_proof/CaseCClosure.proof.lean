-- FILE: Lean/Lehmer/Audit-axle/Audit_proof/CaseCClosure.proof.lean
import Mathlib
...
theorem caseC_empty_audit_from_package
  ...
  intro n hCand hC
  exact audit_caseC_empty_pointwise_from_package
    Params
    ClosureData
    CaseCMainPackage
    LehmerComposite
...
theorem no_LehmerComposite_in_caseC_from_package
  ...
  cases h with
  | intro n hn =>
      exact caseC_impossible_from_package P D X hn.left hn.right
...
theorem audit_caseC_reconstruction_total
    ...
    forall n : Nat,
      InCaseC n ->
        Exists fun _X : AuditCaseCReconstructionPieces n => True := by
  intro n hC
  exact exists_caseC_reconstruction_of_audit_class
    InCaseC
    AuditCaseCReconstructionPieces
    exists_caseCReconstructionPieces_of_inCaseC
    hC