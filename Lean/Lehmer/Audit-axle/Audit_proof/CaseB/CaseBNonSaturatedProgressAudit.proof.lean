-- FILE: Lean/Lehmer/Audit-axle/Audit_proof/CaseB/CaseBNonSaturatedProgressAudit.proof.lean
import Mathlib
...
theorem AuditCaseBExhaustiveLocalOutcome_of_nonSaturatedState
    ...
    AuditCaseBExhaustiveLocalOutcome
      ...
      AuditCaseBEntangledStepData
      C := by
  exact exhaustiveLocalOutcome_of_nonSaturatedState hC
...
theorem exists_entangled_of_boundary_not_purelyGeneric
    ...
    Exists fun _ : AuditCaseBEntangledStepData C => True := by
  exact Exists.intro
    (auditCaseBEntangledStepData_of_boundary C hB hnot)
    True.intro
